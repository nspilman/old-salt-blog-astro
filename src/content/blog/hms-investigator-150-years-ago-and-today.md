---
title: "HMS Investigator – 150 years ago and Today"
date: 2010-07-30T09:02:07
slug: hms-investigator-150-years-ago-and-today
excerpt: "The HMS Investigator was found in remarkably good shape in only 11 meters of water in Mercy Bay along the northern coast of Banks Island in Canada’s western Arctic.  The image to the left is from a  lithograph of the … Continue reading &rarr;"
categories:
  - "Current"
  - "History"
  - "Lore of the Sea"
  - "Ships"
tags:
  - "HMS Investigator"
  - "Mercy Bay"
  - "Parks Canada"
  - "western Arctic"
author: "Rick Spilman"
description: "The HMS Investigator was found in remarkably good shape in only 11 meters of water in Mercy Bay along the northern coast of Banks Island in Canada’s western Arctic.  The image to the left is from a  lithograph of the … Continue reading &rarr;"
---

#gallery-2 { margin: auto; } #gallery-2 .gallery-item { float: left; margin-top: 10px; text-align: center; width: 50%; } #gallery-2 img { border: 2px solid #cfcfcf; } #gallery-2 .gallery-caption { margin-left: 0; } /\* see gallery\_shortcode() in wp-includes/media.php \*/

[![](https://oldsaltblog.com/wp-content/uploads/2010/07/Ship-2-125x99.jpg)](https://oldsaltblog.com/wp-content/uploads/2010/07/Ship-2.jpg)

HMS Investigator Caught in the Ice 1854

[![](https://oldsaltblog.com/wp-content/uploads/2010/07/HMS-Investigator-125x107.jpg)](https://oldsaltblog.com/wp-content/uploads/2010/07/HMS-Investigator.jpg)

Sonar Image of HMS Investigator Photo: Parks Canda

  

The *[HMS Investigator](http://en.wikipedia.org/wiki/HMS_Investigator_\(1848\))* [](http://en.wikipedia.org/wiki/HMS_Investigator_\(1848\))was found in remarkably good shape in only 11 meters of water in Mercy Bay along the northern coast of Banks Island in Canada’s western Arctic.  The image to the left is from a  lithograph of the ship trapped in the ice in 1854 while the image to right is from the Parks Canada sonar scan of the ship on the bottom as she is today.  The [](http://en.wikipedia.org/wiki/HMS_Investigator_\(1848\))*[HMS Investigator](http://en.wikipedia.org/wiki/HMS_Investigator_\(1848\))*, was caught in the ice and sank when on an expedition  to search for [Franklin](http://en.wikipedia.org/wiki/John_Franklin). Click on either thumbnail for a larger image.
