---
title: "A Proposal on HMS Bounty"
date: 2012-05-01T17:32:02
slug: "a-proposal-on-the-bounty"
excerpt: "HMS Bounty recently called at St. Augustine, FL.  During a dockside party on the ship, Dr. Roger Clark Smith, a local historian, decided that there would be no better setting to propose marriage to the fair Christine Whalen, who graciously accepted. Our congratulations to the … Continue reading &rarr;"
categories:
  - "Lore of the Sea"
tags:
  - "Christine Whalen"
  - "Dr. Roger Clark Smith"
  - "HMS Bounty"
  - "st. augustine"
author: "Rick Spilman"
description: "HMS Bounty recently called at St. Augustine, FL.  During a dockside party on the ship, Dr. Roger Clark Smith, a local historian, decided that there would be no better setting to propose marriage to the fair Christine Whalen, who graciously accepted. Our congratulations to the … Continue reading &rarr;"
---

![](https://old-salt-blog-media.s3.us-east-2.amazonaws.com/uploads/2012/05/bountyvow.jpg)

Dr. Roger Clark and fiance Christine Whalen on HMS Bounty Photo: Rhonda Parker

*[HMS Bounty](http://www.tallshipbounty.org/)* recently called at St. Augustine, FL.  During a dockside party on the ship, Dr. Roger Clark Smith, a local historian, decided that there would be no better setting to propose marriage to the fair Christine Whalen, who graciously accepted. Our congratulations to the happy couple.  We only hope that a “no mutiny” clause is added to the vows.  Thanks to [Alaric Bond](http://alaricbond.com/) for passing the news along.

[Local maritime historian proposes at the helm of the HMS Bounty](http://www.examiner.com/article/local-maritime-historian-proposes-at-the-helm-of-the-hms-bounty)
